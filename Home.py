...
st.write("同じ句集に一度でも一登場している場合、ノード間に引力が発生しますが、同じであった回数の多さは位置関係には反映されていません。")
st.write（"この図で見て取るのは、難しいですが同じ句集に登場している頻度はエッジの太さに反映"）
st.write（"大御所達の中で比較すると、東烏は、やはり末席の末席となることがわかります。"）
st.write("派閥の色　「卓池」…黄色、「秋挙」…緑　「木朶」…水色　「祖風」…紫　「無派閥」…紫")
